...
function createConv2(inputfeatures, am, kt1, kt2, kt3, kt4, kt5)
   local conv = nn.Sequential()
   conv:add(nn.VolumetricConvolution(inputfeatures, am, kt1, 11, 11, 1, 4, 4, 0, 2, 2))
   conv:add(nn.ReLU(true))
   conv:add(nn.VolumetricMaxPooling(1, 3, 3, 1, 2, 2))
   ...
   conv:add(nn.ReLU(true))
   conv:add(nn.VolumetricMaxPooling(1, 3, 3, 1, 2, 2))
   len2 = conv:forward(train_x[1]:narrow(1,1,opt.inputfeatures_G4)):storage():size()
   conv:add(nn.View(len2))
   return conv
end
...
featextract:add(createConv2(opt.inputfeatures_G4, opt.f1_G2, opt.kt1_G2, opt.kt2_G2, opt.kt3_G2, opt.kt4_G2, opt.kt5_G2))
featextract:add(nn.Linear(2,2))
model:add(featextract)
model:add(nn.JoinTable(1))
model:add(nn.Dropout(0.5))
model:add(nn.Linear(len+len2+2, 1))
